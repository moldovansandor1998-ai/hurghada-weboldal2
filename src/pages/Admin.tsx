...
export default function Admin() {
  ...
  const [smsText,setSmsText]=useState('Koszonjuk, hogy korabban velunk kirandult! Varjuk ujra Hurghadaban! Tolts fel legalabb 5 sajat elmenyfotot a Hurghada Programok weboldalon a Vendegfotok menupontban, es jovahagyas utan 10 EUR kupont kapsz. A kupon 1 evig ervenyes, es csaladtagnak vagy ismerosnek is atadhatod.')
  const [testPhone,setTestPhone]=useState('')
  const [smsSending,setSmsSending]=useState(false)
  const [smsDiag,setSmsDiag]=useState<Record<string,number>>({})
  const [smsDiagLoading,setSmsDiagLoading]=useState(false)
  const [lastSms,setLastSms]=useState<{id:string;status:string}|null>(null)
  const [bookingPhones,setBookingPhones]=useState<string[]>([])
  const [campaignSentPhones,setCampaignSentPhones]=useState<string[]>([])
  ...
  const checkSmsStatus=async()=>{if(!lastSms)return;const {data:{session}}=await supabase.auth.getSession();const r=await fetch('/api/sms-status?id='+encodeURIComponent(lastSms.id),{headers:{Authorization:'Bearer '+session?.access_token}});const j=await r.json();setLastSms(v=>v?{...v,status:j.status||'PENDING'}:v);setNotice('SMS kézbesítési állapot: '+(j.status||'PENDING')+(j.status_time?' · '+j.status_time:''))}
    const refreshSmsDiagnostics=async()=>{setSmsDiagLoading(true);const {data:{session}}=await supabase.auth.getSession();const r=await fetch('/api/sms-diagnostics',{method:'POST',headers:{Authorization:'Bearer '+session?.access_token,'Content-Type':'application/json'}});const j=await r.json().catch(()=>({}));setSmsDiagLoading(false);if(!r.ok){setNotice('SMS diagnosztika hiba: '+(j.error||'ismeretlen hiba'));return}setSmsDiag(j.summary||{});setNotice('SMS státuszok frissítve: '+j.checked+' üzenet. REJECTED: '+j.rejected)}
